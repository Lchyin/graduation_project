...
void RobotStateRT::unpack(uint8_t * buf) {
	...
	if (version_ > 1.8) {
		...
		//std::vector<bool> digital_output_bits_ = unpackDigitalInputBits(be64toh(digital_output_bits));
		//std::cout <<"output::" <<digital_input_bits_[7] <<std::endl;
	}
	val_lock_.unlock();
	controller_updated_ = true;
	data_published_ = true;
